...
class IIIFManifestGenerator {
  ...
  async saveManifest(book: BookEntry, manifest: IIIFManifest): Promise<void> {
    ...
    console.log(
      `✅ Generated manifest: ${filename} (${manifest.items.length} canvases)`
    );
  }
}
...
async function main() {
  try {
    ...
    const booksWithStructure = libraryData.books.filter(
      (book) => book.structure && book.structure.length > 0
    );
    // .slice(0, 10);
    ...
    console.log(
      `📚 Found ${booksWithStructure.length} books with structure to process:`
    );
    ...
    for (const book of booksWithStructure) {
      try {
        ...
        console.error(
          `❌ Failed to generate manifest for ${book.title}:`,
          error
        );
      }
    }
